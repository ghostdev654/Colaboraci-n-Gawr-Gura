...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  let color = validColors[args[0]?.toLowerCase()] ? args[0].toLowerCase() : 'black';

  if (!cleanText) {
    const colorsText = Object.keys(validColors)
      .map((c, i) => `💠 *${String(i + 1).padStart(2, '0')}*. \`${c}\``)
      .reduce((acc, val, i) => {
        const row = Math.floor(i / 5);
        acc[row] = (acc[row] || []).concat(val);
        return acc;
      }, [])
      .map(row => row.join('   '))
      .join('\n');
    ...
    return m.reply(
`𓆩♡𓆪  *~ Hola! Soy Gura-chan y te ayudaré a crear un sticker mágico ~* 𓆩♡𓆪

🐬 Usa el comando así:
» *${usedPrefix}${command} <color> <texto>*

🌊 Ejemplo:
» *${usedPrefix}${command} pink Hii~ I'm Gura!*

🎨 Colores disponibles:
${colorsText}

⌦ ꒰ Si mencionas a alguien, eliminaré el @ para que se vea más bonito ♡ ꒱

⟣ 𓂃 𖥻 ʚɞ ¡Diviértete creando stickers kawaii con Gura! 🦈💙`
    );
  }
  ...
  try {
    ...
    if (!json.data.result?.image) throw new Error("API falló, nya~");
    ...
    if (stiker) {
      ...
      return m.reply("🐬 Gura-chan no pudo hacer el sticker... Intenta de nuevo porfi~");
    }
  } catch (error) {
    console.error(error);
    return m.reply("💢 Uwaaa~ Algo salió mal con la API de los stickers!");
  }
};
